...
void get_hermitian_matrix(std::vector<long double> eigenvector,	std::vector<std::vector<long double>>& h_matrix)
{
...
void get_hermitian_matrix_inverse(std::vector<long double> eigenvector,	std::vector<std::vector<long double>>& ih_matrix)
{
...
void jordan_gaussian_transform(	std::vector<std::vector<long double>> matrix, std::vector<long double>& eigenvector)
{
	const long double eps = 0.000001; bool eigenv_found = false; //changed Arg
	for (std::uint32_t s = 0; s < matrix.size() - 1 && !eigenv_found; s++)
	{
		std::uint32_t col = s; long double alpha = matrix[s][s];  //changed Arg
		...
		{
			long double gamma = matrix[row][s]; //changed Arg
...
void get_inverse_diagonal_matrix(std::vector<std::vector<long double>> matrix, std::vector<std::vector<long double>>& inv_matrix)
{
...
void get_reduced_matrix(std::vector<std::vector<long double>> matrix,	std::vector<std::vector<long double>>& r_matrix, std::size_t new_size)
{
...
void matrix_by_matrix(std::vector<std::vector<long double>> matrix1, std::vector<std::vector<long double>>& matrix2, std::vector<std::vector<long double>>& matrix3)
{
...
void matrix_transpose(std::vector<std::vector<long double>> matrix1, std::vector<std::vector<long double>>& matrix2)
{
...
void generate_matrix(std::vector<std::vector<long double>>& matrix, std::size_t rows, std::size_t cols)//changed  removed\
...
void compute_evd(std::vector<std::vector<long double>> matrix, std::vector<long double>& eigenvalues, std::vector<std::vector<long double>>& eigenvectors, std::size_t eig_count)
{
	std::size_t m_size = matrix.size();
	std::vector<long double> vec; vec.resize(m_size);
	std::fill_n(vec.begin(), m_size, 1);

	static std::vector<std::vector<long double>> matrix_i;
	...
	std::vector<std::vector<long double>> m; m.resize(m_size);
	for (std::uint32_t row = 0; row < m_size; row++)
		m[row].resize(100);

	long double lambda_old = 0; ///changed Arg
	...
	while (is_eval == false)
	{
		...
		if (index > 0)
		{
			long double lambda = (m[0][index - 1] != 0) ?  (m[0][index] / m[0][index - 1]) : m[0][index];                          //changed Arg ?
	...
	std::vector<std::vector<long double>> matrix_new;

	if (m_size > 1)
	{
		std::vector<std::vector<long double>> matrix_target;
		...
		std::vector<std::vector<long double>> hermitian_matrix;
		get_hermitian_matrix(eigenvector, hermitian_matrix);

		std::vector<std::vector<long double>> ha_matrix_product;
		matrix_by_matrix(hermitian_matrix, matrix, ha_matrix_product);

		std::vector<std::vector<long double>> inverse_hermitian_matrix;
		get_hermitian_matrix_inverse(eigenvector, inverse_hermitian_matrix);

		std::vector<std::vector<long double>> iha_matrix_product;
	...
	if (m_size <= 1)
	{
		...
		{
			long double lambda = eigenvalues[index];                //changed Agr
			std::vector<std::vector<long double>> matrix_target;    //changed arg
			...
			long double eigsum_sq = 0; // changed Arg
...
void svd(std::vector<std::vector<long double>> matrix, std::vector<std::vector<long double>>& s,
	std::vector<std::vector<long double>>& u, std::vector<std::vector<long double>>& v)
{
	std::vector<std::vector<long double>> matrix_t;
	matrix_transpose(matrix, matrix_t);

	std::vector<std::vector<long double>> matrix_product1;
	matrix_by_matrix(matrix, matrix_t, matrix_product1);

	std::vector<std::vector<long double>> matrix_product2;
	matrix_by_matrix(matrix_t, matrix, matrix_product2);

	std::vector<std::vector<long double>> u_1;
	std::vector<std::vector<long double>> v_1;
	...
	std::vector<std::vector<long double>> s_inverse;
	get_inverse_diagonal_matrix(s, s_inverse);

	std::vector<std::vector<long double>> av_matrix;
